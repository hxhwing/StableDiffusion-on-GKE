# Dockerfile.Stable --> Dockerfile with upscaler only
#RealESRGAN_x4plus
#ESRGAN
...
RUN wget https://github.com/xinntao/Real-ESRGAN/releases/download/v0.1.0/RealESRGAN_x4plus.pth --directory-prefix /content/stable-diffusion-webui/models/RealESRGAN/
RUN wget https://github.com/cszn/KAIR/releases/download/v1.0/ESRGAN.pth --directory-prefix /content/stable-diffusion-webui/models/ESRGAN/


## Enable API ##
RUN echo "export COMMANDLINE_ARGS=\"--api\"" >> /content/stable-diffusion-webui/webui-user.sh
RUN chmod +x /content/stable-diffusion-webui/webui-user.sh
...
RUN cd stable-diffusion-webui && python launch.py --skip-torch-cuda-test --api
...
CMD cd /content/stable-diffusion-webui && python webui.py --xformers --listen --enable-insecure-extension-access --gradio-queue --api